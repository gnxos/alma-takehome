...
def verify_credentials(email: str, password: str) -> bool:
    if email.strip().lower() != config.ATTORNEY_EMAIL.strip().lower():
        return False
    # Timing-safe: only the password is a secret worth guarding against a
    # timing side-channel here.
    return secrets.compare_digest(password.encode(), config.ATTORNEY_PASSWORD.encode())
...
def get_current_attorney_email(
    ...
    try:
        payload = jwt.decode(access_token, config.JWT_SECRET, algorithms=[ALGORITHM])
